...
for i in range(len(test_data_paths)):
	...
	if anomaly_preds != -1:
		...
		obj_detection_prediction_file_path = "prediction_" + result + "_" + label + ".jpg"
		model.predict(input_image_path, confidence=40, overlap=30).save(obj_detection_prediction_file_path)
